                  ...
                  <thead>
                      ...
                      <tbody>
                        <tr v-for="(item, index) in staffs2.content" :key="index">
                          <td>
                            <input v-if="item.status === 3" type="checkbox" :value="item.id" v-model="selected" :ref="item.id" required />
                            <input v-if="(item.status === 2)" type="checkbox" :value="item.id" v-model="selected" :ref="item.id" disabled />
                    ...
                    <el-pagination style="text-align: right" background layout="prev, pager, next" :page-count="count2"
                      :page-size="pageSize2" :page-sizes="pageSizes2" @current-change="handlePageChange2" hide-on-single-page>
                  </el-pagination>
              </div>
            </div>
          </div>
                  ...
                  <div>
                    <div class="mb-3">
                      ...
                      <input @keyup="keyupStaffUpdate" v-model="staffUpdate.welfareMoney" @blur="keyupStaffUpdate"
                        ...
                        placeholder="Tiền hỗ trợ phúc lợi" disabled/>
                    </div>
                  </div>
  ...
  <!-- 
              ...
              <div style="text-align: center">
                <button @click.prevent="create" class="btn btn-danger" style="margin: 10px;">
  ...
  export default {
    ...
    computed: {
      ...
      selectAll2: {
        get: function () {
          return this.staffs2.content ? this.selected.length == this.staffs2.content.length : false;
        },
        set: function (value) {
          var selected = [];
